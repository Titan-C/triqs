...
try : 
    ...
    wrapper_mako, wrapper_target, header_mako, header_target, install_dir = sys.argv[1:6]

except: #only called by autodoc
    pass

# Directories where to find the headers generated by other modules
module_path_list = sys.argv[6:] 
...
class module_ :
    ...
    def _generate_py_converter_header(self, mako_template, wrap_file) :
        """
         generate the header from template file py_converter.hpp
         if wrap_file ends with .to_be_installed,
         self.app_name is given to the renderer to prepend
         self.app_name to the includes (with additional <...>)
        """
        self._prepare_for_generation()
        tpl = Template(filename=mako_template)
